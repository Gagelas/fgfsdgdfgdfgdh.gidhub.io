...
class SupportController extends Controller {
	 ...
	 const SUPPORT_NAME = 'Support @ CSGO-BEEN.RU';  // Imja kotorie pokazivaetsa kogda admin otvechaet v tikete
	 const SUPPORT_TITLE = 'ТЕХНИЧЕСКАЯ ПОДДЕРЖКА'; // Nazvanie dlja stronici tiketa i supporta
	 const TITLE_UP = 'Поддержка | '; // Etot title pakazivaetsa v verhu gde vkladka 

	public function support(Request $request) {
		
		$admin = $this->user->is_admin;
		$moderator = $this->user->is_moderator;
		$limit = $this->user->request;
		$users = $this->user->id;
		$usersname = $this->user->username;
		$usersava = $this->user->avatar;
		$error = NULL;
		$error2 = NULL;
		...
		if ($admin == 1 || $moderator == 1) {
			...
			$tickets = \DB::table('support')->where('user_id',$this->user->id)->orderBy('status')->get();
		}
...
if (!$error && !$error2) {
	...
	public function ticket($ticketId){
		...
		{			
	$error = NULL;
			$tmess = \DB::table('users')->where('steamid64',$this->user->id)->get();
